...
const applyTimeoutLogic = (db, patient) => {
  // Return a Promise to allow asynchronous handling of the timeout logic
  return new Promise((resolve, reject) => {
    // Timeout logic for updating patient status to "WARDED" and ward status to "OCCUPIED"
    setTimeout(() => {
      updatePatientStatus(db, patient, "WARDED");
      updateWardStatus(db, patient, "OCCUPIED");
    }, 5000); // 5 seconds in milliseconds

    // Timeout logic for updating patient status to "DISCHARGED" and ward status to "DISCHARGED PENDING SANITIZING"
    setTimeout(() => {
      updatePatientStatus(db, patient, "DISCHARGED");
      updateWardStatus(db, patient, "DISCHARGED PENDING SANITIZING");
    }, 10000); // 10 seconds in milliseconds

    // Timeout logic for updating ward status to "SANITIZING"
    setTimeout(() => {
      updateWardStatus(db, patient, "SANITIZING");
    }, 15000); // 15 seconds in milliseconds

    // Timeout logic for updating ward status to "AVAILABLE"
    setTimeout(() => {
...
const myTimer = () => {
  const d = new Date();
  document.getElementById("timerValue").innerHTML = d.toLocaleTimeString();
};
...
/>
    ...
    >
      ...
      ><span id="ward-status">${patient.wardStatus}</span>
    </p>
    <p
    class="border-2 bg-white border-gray-200 rounded-md px-2 py-1 mb-2 flex justify-between"
  >
  ...
  <p id="timer" class="border-2 bg-white border-gray-200 rounded-md px-2 py-1 mb-2 flex justify-between">
  <span class="bg-black text-white px-2 py-1 rounded-md">Timer:</span><span id="timerValue">00.00.00</span>
</p>
  </div>
</div>
  `;
...
const intensiveWardAllocation = async (db, patient) => {
  ...
  >
    ...
    class="border-2 bg-white border-gray-200 rounded-md px-2 py-1 mb-2 flex justify-between"
  >
    <span class="bg-black text-white px-2 py-1 rounded-md">Patient Status:</span> ${patient.status}</span>
  </p>
      <p id="timer"
        class="border-2 bg-white border-gray-200 rounded-md px-2 py-1 mb-2 flex justify-between"
      >
...
const infectiousWardAllocation = (db, patient) => {
  ...
  >
    ...
    setTimeout(() => {
      updatePatientStatus(db, patient, "DISCHARGED");
      updateWardStatus(db, patient, "DISCHARGED PENDING SANITIZING");
    }, 10000); // 2 minute in milliseconds
